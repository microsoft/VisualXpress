...
namespace Microsoft.VisualXpress.Perforce
{
	public static class Process
	{
		...
		public static Config[] P4vFavoriteConnections
		{
			get
			{
				SortedDictionary<string, Config> connections = new SortedDictionary<string, Config>();
				try
				{
					string settingsFile = Process.P4vConnectionsFile;
					if (System.IO.File.Exists(settingsFile))
					{
						var document = new System.Xml.XPath.XPathDocument(settingsFile);
						string connectionDefPath = "/ConnectionDefList[@varName=\"connectiondeflist\"]/ConnectionDef";
						foreach (System.Xml.XPath.XPathNavigator connectionDefNode in document.CreateNavigator().Select(connectionDefPath))
						{
							Config config = new Config
							{
								Port = connectionDefNode.SelectSingleNode("P4Port")?.Value,
								User = connectionDefNode.SelectSingleNode("User")?.Value,
								Client = connectionDefNode.SelectSingleNode("Workspace")?.Value,
							};
							if (config.IsEmptyConnection == false)
		...
		public static Config[] P4vConnections
		{
			get
			{
				...
				foreach (var c in P4vRecentConnections)
					connections[c.ToConnectionString()] = c;
				foreach (var c in P4vFavoriteConnections)
					connections[c.ToConnectionString()] = c;
				return connections.Values.ToArray();
			}
		}

		public static Config[] GetKnownConnections(bool includeAllClients = false)
		{
			...
			Config currentConnection = Perforce.Process.Connection();
			if (currentConnection.IsEmptyConnection == false)
				connections[currentConnection.ToConnectionString()] = currentConnection;
			...
			if (includeAllClients)
			{
				...
				Parallel.ForEach(ports.Values, (Config c) =>
				{
			...
			Parallel.ForEach(connections.Values, (Config c) =>
			{
				Config nc = NormalizeConnection(c);
				lock (normConnections)
				{
		...
		public static string P4vUserSettingsFolder
		{
			get { return String.Format("{0}\\.p4qt", Environment.GetFolderPath(Environment.SpecialFolder.UserProfile)); }
		}

		public static string P4vApplicationSettingsFile
		{
			get { return String.Format("{0}\\ApplicationSettings.xml", P4vUserSettingsFolder); }
		}

		public static string P4vConnectionsFile
		{
			get { return String.Format("{0}\\connections.xml", P4vUserSettingsFolder); }
		}
	...
	public class Config
	{
		...
		{
			get { return String.IsNullOrEmpty(this.Port) && String.IsNullOrEmpty(this.Client) && String.IsNullOrEmpty(this.User); }
		}
