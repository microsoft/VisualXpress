...
namespace Microsoft.VisualXpress
{
	...
	public sealed class Package : AsyncPackage, IVsSolutionLoadEvents, IOleCommandTarget, IVsPersistSolutionOpts, IDisposable
	{
		...
		public IVsSolutionBuildManager2 GetSolutionBuildManager()
		{
			IVsSolutionBuildManager2 SolutionBuildManager = null;
		...
		private void ExecuteAction(Action action, bool wait)
		{
			...
			else
			{
				...
				{
					await JoinableTaskFactory.SwitchToTaskPoolThreadAsync();
					action();
				}).FileAndForget(Telemetry.EventFaultName("Package.ExecuteAction"));
			}
		}
